package Proyecto03.JuegoBuscaminas;
...
import Proyecto03.datosUsuario.GestionUsuario;
...
public class TablaBuscaminas {
	...
	private Casilla[][] tablaBuscaminas;//Tabla con casillas
	...
	public TablaBuscaminas(Usuario usuario) { //Constructor principal
		...
		this.tablaBuscaminas = new Casilla[fila][columna]; //Tabla buscaminas
		numeroMarcas = 0;
		sacarTop10();
	}
	...
	private void generarNuevoJuego() {
		this.tablaBuscaminas = new Casilla[this.fila][this.columna]; //Destruye la tabla
	...
	private Casilla getCasilla(int posX, int posY) { //Toma la casilla correspondiente
	...
	private void generarMinas(int cantidad) { //Recursivo genera las minas
		...
		try { //Va a dar error cada que verifique que no puede sacar valor de una casilla Null
			...
			this.tablaBuscaminas[posMinaFila][posMinaColumna] = new Casilla(true);//Mina
		...
		for(int i=0;i<this.fila;i++) {
			for(int j=0;j<this.columna;j++) {
				if(this.tablaBuscaminas[i][j] == null) {//Si es null
					this.tablaBuscaminas[i][j] = new Casilla(false);//Casilla nueva vacia
				}
	...
	private boolean ganar() {//Gana si todas las casillas sin minas estan destapadas
		for(int i=0;i<this.fila;i++) {
			for(int j=0;j<this.columna;j++) {//Encuentra una casilla destapada que no hay mina
				Casilla c = this.tablaBuscaminas[i][j];
